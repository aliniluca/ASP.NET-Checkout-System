...
namespace WebAppRestaurantDemoApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            List<Models.Transaction> listTransactions = new List<Models.Transaction>();
            ...
            while (true)
            {
                try
                {
                    var transactions = restaurantDBEntities.Transactions.Single(model => model.TransactionId == index);
            ...
            //decimal UnitPrice = restaurantDBEntities.Items.Single(model => model.ItemId == itemId).ItemPrice;
            return Json(listTransactions.ToArray(), JsonRequestBehavior.AllowGet);
        }
